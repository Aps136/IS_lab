...
def hill_cipher_2x2_encrypt(plaintext, key_matrix):
    """Encrypts plaintext using a 2x2 Hill cipher key."""
    ...
    ciphertext = ""
    for i in range(0, len(text_to_num), 2):
        # Create the plaintext vector
        p_vector = np.array(text_to_num[i:i + 2]).reshape(2, 1)
...
def hill_cipher_2x2_decrypt(ciphertext, key_matrix):
    """Decrypts ciphertext using a 2x2 Hill cipher key."""
    # Check if the key matrix is invertible mod 26
    det = int(np.round(np.linalg.det(key_matrix)))
    det_inv = mod_inverse(det % 26, 26)
    if det_inv is None:
    ...
    for i in range(0, len(text_to_num), 2):
        ...
        c_vector = np.array(text_to_num[i:i + 2]).reshape(2, 1)
...
if __name__ == '__main__':
    # A 2x2 key matrix that is invertible mod 26
    key = np.array([[9, 4],
                    [5, 7]])
    ...
    # Encryption
    encrypted_msg = hill_cipher_2x2_encrypt(plaintext_msg, key)
    print(f"Plaintext: {plaintext_msg}")
    print(f"Encrypted message: {encrypted_msg}")

    # Decryption
    decrypted_msg = hill_cipher_2x2_decrypt(encrypted_msg, key)
